...
describe('playerCrafting definitions', () => {
  it('exposes the starter recipe registry with nearby-station requirements', () => {
    ...
    expect(getPlayerCraftingRecipeDefinition('wood-wall').requiredStationId).toBeNull();
    ...
    expect(getPlayerCraftingRecipeDefinition('axe').requiredStationId).toBe('anvil');
    ...
    expect(isPlayerCraftingRecipeId('wood-wall')).toBe(true);
    ...
    expect(isPlayerCraftingRecipeId('axe')).toBe(true);
...
describe('evaluatePlayerCraftingRecipe', () => {
  ...
  it('keeps the wood-wall recipe craftable without a nearby station when wood is available', () => {
    const evaluation = evaluatePlayerCraftingRecipe({
      inventoryState: createPlayerInventoryState({
        hotbar: [{ itemId: 'wood', amount: 1 }, ...Array.from({ length: 9 }, () => null)]
      }),
  ...
  it('blocks station recipes until the matching nearby station is placed', () => {
    ...
    const axeWithoutAnvil = evaluatePlayerCraftingRecipe({
      inventoryState,
      recipeId: 'axe',
      playerState: createPlayer(),
      world: createWorld({
    ...
    const axeWithAnvil = evaluatePlayerCraftingRecipe({
      inventoryState,
      recipeId: 'axe',
      playerState: createPlayer(),
      world: createWorld({
    ...
    expect(axeWithoutAnvil).toMatchObject({
      hasIngredients: true,
      stationInRange: false,
      blocker: 'missing-station',
      craftable: false
    });
    expect(axeWithAnvil).toMatchObject({
      hasIngredients: true,
      stationInRange: true,
      blocker: null,
      craftable: true
...
describe('tryCraftPlayerRecipe', () => {
  ...
  it('crafts four wood walls by consuming one wood without a nearby station', () => {
    const result = tryCraftPlayerRecipe({
      inventoryState: createPlayerInventoryState({
        hotbar: [{ itemId: 'wood', amount: 2 }, ...Array.from({ length: 9 }, () => null)]
      }),
  ...
  it('crafts a starter axe from copper bars only when a nearby anvil is available', () => {
    const inventoryState = createPlayerInventoryState({
      hotbar: [
