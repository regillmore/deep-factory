...
const testRuntime = vi.hoisted(() => {
  ...
  return {
    ...
    latestRendererRenderFrameState: null as null | {
      ...
      grapplingHookCurrentPositions: Array<{ id: number; position: { x: number; y: number } }>;
  ...
  Renderer: class {
    ...
    render(_camera?: unknown, frameState?: unknown): void {
      ...
      const arrowCurrentPositions = Array.isArray(renderState.entities)
        ...
        : [];
      const grapplingHookCurrentPositions = Array.isArray(renderState.entities)
        ? renderState.entities
            .filter((entity) => entity?.kind === 'grappling-hook')
            .flatMap((entity) => {
              const snapshot = entity?.snapshot?.current;
              if (
                !snapshot?.position ||
...
describe('main.ts shell state orchestration', () => {
  ...
  it('places a closed door pair from the selected hotbar slot while the full debug-edit panel is hidden', async () => {
    setPersistedWorldSaveWithInventory(
      createPlayerInventoryState({
        hotbar: [
          { itemId: 'pickaxe', amount: 1 },
    ...
    testRuntime.rendererTileIdsByWorldKey.set(worldTileKey(5, -1), 1);
    testRuntime.rendererTileIdsByWorldKey.set(worldTileKey(4, 0), 1);
    testRuntime.rendererTileIdsByWorldKey.delete(worldTileKey(4, -2));
  ...
  it('spawns an in-flight grappling-hook entity and clears it once the hook outruns its maximum range', async () => {
    const standalonePlayerState = createPlayerState({
      position: { x: 8, y: 28 },
      facing: 'right'
    });
    ...
    testRuntime.playerItemUseRequests = [
      {
        worldTileX: 30,
        worldTileY: 0,
        worldX: playerFocusPoint.x + DEFAULT_GRAPPLING_HOOK_MAX_RANGE + 80,
        worldY: playerFocusPoint.y,
        pointerType: 'mouse'
      }
    ];
    ...
    const hookPositions = testRuntime.latestRendererRenderFrameState?.grapplingHookCurrentPositions ?? [];
    expect(hookPositions).toHaveLength(1);
    expect(hookPositions[0]?.position.x).toBeCloseTo(
      playerFocusPoint.x + DEFAULT_GRAPPLING_HOOK_SPEED * (1 / 60),
      6
    );
    expect(hookPositions[0]?.position.y).toBeCloseTo(playerFocusPoint.y, 6);
    expect(getHotbarOverlaySlotAmountLabel(6).textContent).toBe('HOOK');

    testRuntime.playerItemUseRequests = [];
    runFixedUpdate(DEFAULT_GRAPPLING_HOOK_MAX_RANGE / DEFAULT_GRAPPLING_HOOK_SPEED);
    runRenderFrame(1000, 1);
  ...
  it('pulls the standalone player toward a latched grappling-hook anchor and clears it after release', async () => {
    ...
    const latchedHookPositions =
      testRuntime.latestRendererRenderFrameState?.grapplingHookCurrentPositions ?? [];
    expect(latchedHookPositions).toHaveLength(1);
    expect(latchedHookPositions[0]?.position.x).toBeCloseTo(28, 6);
    expect(latchedHookPositions[0]?.position.y).toBeCloseTo(playerFocusPoint.y, 6);
    expect(testRuntime.latestRendererRenderFrameState?.standalonePlayerCurrentPosition).toEqual({
      x: 8,
      y: 32
    });
    ...
    const pullingHookPositions =
      testRuntime.latestRendererRenderFrameState?.grapplingHookCurrentPositions ?? [];
    expect(pullingHookPositions).toHaveLength(1);
    expect(pullingHookPositions[0]?.position.x).toBeCloseTo(28, 6);
    expect(pullingHookPositions[0]?.position.y).toBeCloseTo(playerFocusPoint.y, 6);
    expect(testRuntime.latestRendererRenderFrameState?.standalonePlayerCurrentPosition).toEqual({
      x: 17,
      y: 32
    });
  ...
  it('clears an active grappling-hook traversal from a follow-up touch use', async () => {
    const standalonePlayerState = createPlayerState({
      position: { x: 8, y: 32 },
      grounded: true,
      facing: 'right'
  ...
  it('clears a latched grappling-hook traversal when paused-menu New World replaces the session', async () => {
    const standalonePlayerState = createPlayerState({
      position: { x: 8, y: 32 },
      grounded: true,
      facing: 'right'
    ...
    const freshWorldRenderFrameState = testRuntime.latestRendererRenderFrameState as
      | {
          grapplingHookCurrentPositions: Array<{
            id: number;
            position: { x: number; y: number };
          }>;
        }
  ...
  it('clears an in-flight grappling-hook traversal when a paused-menu import replaces the session', async () => {
    ...
    expect(restoredWorldRenderFrameState?.grapplingHookCurrentPositions).toEqual([]);
    expect(getHotbarOverlaySlotAmountLabel(6).textContent).toBe('');
    expect(getHotbarOverlaySlotButton(6).title).not.toContain('hook active');
  });
