...
describe('buildDebugEditStatusStripModel', () => {
  ...
  it('shows pinned inspect metadata with a repin hint when no separate hover target is present', () => {
    ...
    expect(model.hoverText).toBe(
      'Pinned: lava pool (#9) @ 12,-4 chunk:0,-1 local:12,28 | solid:off | light:on | liquid:lava | liquidGroup:lava | liquidMask:NE-W (11) | liquidSrc:atlasIndex 15 | liquidUv:0.5,0.75..0.667,1 | liquidPx:48,48..64,64'
    );
  ...
  it('deduplicates compact inspect metadata when hovered and pinned targets match', () => {
    ...
    expect(model.hoverText).toBe(
      'Shared: lava pool (#9) @ 12,-4 chunk:0,-1 local:12,28 | solid:off | light:on | liquid:lava | liquidGroup:lava | liquidMask:NE-W (11) | liquidSrc:atlasIndex 15 | liquidUv:0.5,0.75..0.667,1 | liquidPx:48,48..64,64'
    );
