...
const formatPlayerCombatLine = (player: DebugOverlayPlayerTelemetry | null): string => {
  ...
  const respawnText =
    typeof player.respawnSecondsRemaining === 'number' &&
    Number.isFinite(player.respawnSecondsRemaining)
      ? `${player.respawnSecondsRemaining.toFixed(2)}s`
      : null;
  const deathHoldText =
    player.deathHoldStatus === 'none' ||
    player.deathHoldStatus === 'holding' ||
    player.deathHoldStatus === 'respawned'
      ? player.deathHoldStatus
      : null;
  const breathText =
  ...
  const maxHealthText =
    typeof player.maxHealth === 'number' && Number.isFinite(player.maxHealth)
      ...
      : null;
  const manaText =
    typeof player.mana === 'number' && Number.isFinite(player.mana)
      ? `${Math.round(player.mana)}`
      : typeof player.maxMana === 'number' && Number.isFinite(player.maxMana)
        ? 'n/a'
        : null;
  const maxManaText =
    typeof player.maxMana === 'number' && Number.isFinite(player.maxMana)
  ...
  if (manaText !== null) {
    segments.push(`mana:${maxManaText === null ? manaText : `${manaText}/${maxManaText}`}`);
  }
  ...
  if (deathHoldText !== null) {
    segments.push(`deathHold:${deathHoldText}`);
  }
  if (breathText !== null) {
