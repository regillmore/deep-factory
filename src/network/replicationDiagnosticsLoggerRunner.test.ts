...
describe('createAuthoritativeClientReplicationDiagnosticsLoggerRunner', () => {
  it('returns a ready-to-poll runner that stays silent before the cadence due tick', () => {
    ...
    const loggerRunner = createAuthoritativeClientReplicationDiagnosticsLoggerRunner({
  ...
  it('runs the mixed logger bundle on due ticks while returning the cadence emission', () => {
    const registry = new AuthoritativeClientReplicationDiagnosticsRegistry();
    registry.setSnapshot('client-alpha', createPopulatedSnapshot(5));
  ...
  it('preserves sink-owned mutation isolation through the runner wrapper', () => {
    const registry = new AuthoritativeClientReplicationDiagnosticsRegistry();
    ...
    const result = loggerRunner.poll({ tick: 8 });
  ...
  it('reuses cadence rescheduling so late polls still run once and stay quiet until the next due tick', () => {
    const registry = new AuthoritativeClientReplicationDiagnosticsRegistry();
    ...
    expect(loggerRunner.poll({ tick: 14 })).toEqual({
      emitted: true,
      nextDueTick: 20,
      payload: expectedPayload,
      logLines: expectedLines,
    ...
    expect(loggerRunner.getNextDueTick()).toBe(20);
    expect(loggerRunner.poll({ tick: 19 })).toEqual({
      emitted: false,
      nextDueTick: 20,
      payload: null,
      logLines: null,
