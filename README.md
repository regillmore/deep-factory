...
- App shell: wait for boot to finish, choose `Enter World` from the main menu, use `Main Menu` in-world to pause back to the menu without discarding the current session, then use `Resume World` to continue that same session; the top-right `Recenter Camera` action or `C` resets camera follow framing, the `Debug HUD` toggle or `H` shortcut shows and hides the text telemetry, the `Edit Panel` toggle shows and hides the full `Debug Edit` control surface, the `Edit Overlays` toggle or `V` shortcut shows and hides the compact debug-edit overlays, and the `Spawn Marker` toggle or `M` shortcut shows and hides the standalone player spawn overlay.
