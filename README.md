...
- App shell: wait for boot to finish, choose `Enter World` from the main menu, use `Main Menu` in-world or press `Q` to pause back to the menu without discarding the current session, then use `Resume World (Enter)` or press `Enter` on the paused menu to continue that same session, or use `New World (N)` from the paused menu or press `N` there to discard it and boot a fresh world immediately; the top-right `Recenter Camera` action or `C` resets camera follow framing, the `Debug HUD` toggle or `H` shortcut shows and hides the text telemetry, the `Edit Panel` toggle or `G` shortcut shows and hides the full `Debug Edit` control surface, the `Edit Overlays` toggle or `V` shortcut shows and hides the compact debug-edit overlays, and the `Spawn Marker` toggle or `M` shortcut shows and hides the standalone player spawn overlay.
