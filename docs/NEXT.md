...
563. Replace the single-leaf small-tree-leaf atlas sprite: committed placeholder multi-leaf foliage block art aligned with the new tall-grass and surface-flower palette, plus authored-atlas regressions.
565. Persist small-tree growth cadence after streamed procedural tracking: save and restore the growth timer plus next window index so planted saplings resume their deterministic schedule after paused-session reload, and add save/load regressions.
