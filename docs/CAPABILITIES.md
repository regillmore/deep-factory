...
- App shell now has explicit `boot`, `main menu`, and `in-world` states; renderer and input bootstrap run behind the boot screen, the main menu exposes an `Enter World` action once startup finishes, later switches that same primary action to `Resume World` after a session has been started and paused, adds a secondary `New World` action in that paused state plus a matching desktop `N` shortcut that starts a fresh world only from that paused menu, and the fixed-step simulation starts only after the first `in-world` transition.
...
- App shell: click `Enter World` from the main menu after boot finishes, then use `Resume World` after returning to the menu from an active session or `New World` from that paused menu to discard the current session and boot a fresh world immediately; while that paused menu is visible, press `N` to trigger the same fresh-world reset path from the keyboard.
