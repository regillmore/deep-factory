...
- Renderer atlas initialization ships with an authored atlas at `public/atlas/tile-atlas.png` exposed at `${import.meta.env.BASE_URL}atlas/tile-atlas.png`, so project-site builds fetch it under `/deep-factory/atlas/tile-atlas.png`, then falls back to a generated placeholder atlas whose canvas size and painted regions derive from `src/world/authoredAtlasLayout.ts` if that asset cannot be fetched or decoded.
...
- Deployment regression coverage temp-builds the production app, verifies the `/deep-factory/` bundle URLs, and byte-compares the copied authored atlas output at `dist/atlas/tile-atlas.png` against `public/atlas/tile-atlas.png`.
