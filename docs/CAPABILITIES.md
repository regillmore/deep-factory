...
- App shell now has explicit `boot`, `main menu`, and `in-world` states; renderer and input bootstrap run behind the boot screen, the main menu exposes an `Enter World` action once startup finishes, later switches that same primary action to `Resume World` after a session has been started and paused, and the fixed-step simulation starts only after the first `in-world` transition.
...
- App shell: click `Enter World` from the main menu after boot finishes, then use `Resume World` after returning to the menu from an active session.
