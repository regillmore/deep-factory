...
- Debug overlay shows FPS, rendered chunk count, mesh telemetry, loaded atlas source kind (`authored` or `placeholder`), live pointer inspect data (`client`, `canvas`, `world`, hovered tile name plus tile ID, `tile`, `chunk`, `chunk-local`, `solid`, `light`, `liquid`), and a separate pinned tile metadata line when inspect pin is active.
