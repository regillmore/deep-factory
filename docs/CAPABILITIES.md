...
- App shell: click `Enter World` from the main menu after boot finishes, then use `Resume World` after returning to the menu from an active session or `New World` from that paused menu to discard the current session and boot a fresh world immediately.
