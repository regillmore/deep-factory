...
- Deployment regression coverage temp-builds the production app, verifies the `/deep-factory/` bundle URLs, scans emitted `index.html` to reject both root-relative `"/assets/..."` and double-prefixed `"/deep-factory/deep-factory/assets/..."` script and stylesheet URLs, scans every emitted CSS asset and every emitted JavaScript asset to assert that the exact legacy root-relative `'/atlas/tile-atlas.png'` literal appears nowhere in those outputs while the exact joined runtime atlas fetch URL `/deep-factory/atlas/tile-atlas.png` still appears exactly once across the full emitted JS set, and byte-compares the copied authored atlas output at `dist/atlas/tile-atlas.png` against `public/atlas/tile-atlas.png`.
