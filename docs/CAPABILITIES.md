...
- Debug overlay shows FPS, rendered chunk count, mesh telemetry, loaded atlas source kind plus atlas dimensions, atlas validation status, the resolved standalone player spawn tile plus world coordinates, live standalone player state telemetry (`position`, `velocity`, `grounded`, `facing`), live pointer inspect data (`client`, `canvas`, `world`, hovered tile name plus tile ID, `tile`, `chunk`, `chunk-local`, `solid`, `light`, `liquid`), and a separate pinned tile metadata line when inspect pin is active.
- Debug overlay also reports per-frame animated chunk UV upload cost as upload count, changed animated-quad count, and total uploaded bytes, so metadata-driven tile animation churn stays visible while authored frame sets grow.
