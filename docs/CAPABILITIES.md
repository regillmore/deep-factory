...
- App shell: click `Enter World` from the main menu after boot finishes, then use `Resume World (Enter)` after returning to the menu from an active session or `New World` from that paused menu to discard the current session and boot a fresh world immediately; while that paused menu is visible, press `Enter` to trigger the same resume path from the keyboard or `N` to trigger the fresh-world reset path.
