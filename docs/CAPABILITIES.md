...
- App shell now has explicit `boot`, `main menu`, and `in-world` states; renderer and input bootstrap run behind the boot screen, the main menu exposes an `Enter World` action once startup finishes, later switches that same primary action to `Resume World (Enter)` after a session has been started and paused, lets desktop `Enter` trigger that paused-menu resume path, adds a secondary `New World (N)` action in that paused state plus a matching desktop `N` shortcut that starts a fresh world only from that paused menu, visibly warns in paused-menu copy that `New World` discards the current session before booting a fresh procedural world, repeats that same destructive reset guidance plus the `N` shortcut in the paused-menu shell button tooltip, and the fixed-step simulation starts only after the first `in-world` transition.
- Choosing `New World` from the paused main menu abandons the current world session, resets world/player/camera/undo runtime state plus shell visibility toggles back to first-start defaults, and immediately boots a fresh procedural world without reinitializing the already-loaded renderer or discarding persisted debug-edit preferences.
- Once in-world, the shell keeps lightweight top-right `Main Menu (Q)`, `Recenter Camera`, `Debug HUD`, `Show Edit Panel (G)` or `Hide Edit Panel (G)`, `Edit Overlays`, and `Spawn Marker` actions; the `Main Menu (Q)` action, and matching desktop `Q` shortcut, return to the main menu without rebuilding the initialized renderer or world session, the recenter action, and matching desktop `C` shortcut, snap the camera back to the standalone player's body-center focus and clear any accumulated manual follow offset, the HUD toggle and matching desktop `H` shortcut show or hide the text telemetry overlay, the edit-panel toggle and matching desktop `G` shortcut show or hide the full `Debug Edit` control surface independently from the compact overlays, the edit-overlay toggle plus matching desktop `V` shortcut hide or restore the compact status strip, hovered or pinned inspect outlines, and one-shot preview overlays, and the spawn-marker toggle plus matching desktop `M` shortcut hide or restore the resolved standalone player spawn AABB, support, and anchor overlay without discarding the active world session.
...
- The default tile set now also includes placeholder `water` and `lava` brush tiles with dedicated `liquidRender` metadata, separate liquid connectivity groups, and animated liquid frame sequences; water resolves distinct placeholder surface, edge, and body sources from liquid masks through two-frame direct-`uvRect` animation in dedicated authored regions, and lava now resolves matching surface, edge, and body source classes by animating mask-specific direct-`uvRect` crops over the existing authored lava frames.
...
- Debug overlay shows FPS, rendered chunk count, mesh telemetry, loaded atlas source kind plus atlas dimensions, atlas validation status, the resolved standalone player spawn tile plus world coordinates, live standalone player state telemetry (`position`, `velocity`, `grounded`, `facing`), the current standalone player placeholder pose label chosen from the renderer-facing pose resolver, the renderer-side ceiling-bonk presentation latch state while that brief post-contact hold remains active, the latest standalone player grounded-transition event (`jump`, `fall`, or `landing` with resolved position and velocity), the latest standalone player facing-transition event (`left->right` or `right->left` with resolved position and velocity), the latest standalone player auto-respawn event when embedded-tile recovery reuses the resolved spawn (`embedded` with spawn tile, position, and velocity), the latest standalone player wall-contact and ceiling-contact transition events (`blocked` or `cleared` with tile, position, and velocity, plus resolved `left` or `right` side for wall-contact events), live standalone player collision AABB telemetry (`min`, `max`, `size`), live standalone player camera-follow telemetry (`cam`, `camTile`, `camChunk`, `zoom`, `focus`, `focusTile`, `focusChunk`, `focusLocal`, `offset`), live standalone player collision-contact telemetry (`support`, `wall`, `ceiling` with tile coordinates and IDs when present, and resolved `left` or `right` side for wall contacts), live standalone player input-intent telemetry (`move`, `jumpHeld`, `jumpPressed`), live pointer inspect data (`client`, `canvas`, `world`, hovered tile name plus tile ID, `tile`, `chunk`, `chunk-local`, `solid`, `light`, `liquid`), plus resolved liquid connectivity-group labels, liquid cardinal masks, liquid animation frame indices, frame counts, frame durations, elapsed-in-frame times, normalized frame-progress percentages, remaining-in-frame times, loop durations, elapsed-in-loop times, normalized loop-progress percentages, remaining-in-loop times, and current liquid variant source labels, UV rects, and atlas pixel bounds for liquid tiles, and a separate pinned tile metadata line when inspect pin is active.
- Debug overlay also reports resident animated chunk mesh counts plus recorded animated-quad totals with separate `nonLiquid` and `liquid` quad breakdowns, alongside per-frame animated chunk UV upload cost as upload count, changed animated-quad totals, matching `nonLiquid` and `liquid` upload breakdowns, and total uploaded bytes, so both steady-state authored animation footprint and frame-boundary UV churn stay visible.
...
- The shell-level `Show Edit Panel (G)` or `Hide Edit Panel (G)` toggle hides or restores the full `Debug Edit` control surface independently from the compact overlays, and its shell-button tooltip now repeats the matching desktop `G` shortcut alongside the existing show or hide copy; the panel starts hidden on desktop-oriented sessions and visible on touch-oriented sessions where the on-screen controls are the primary editing surface.
...
- Hovered and touched inspect readouts include tile name, tile ID, world plus chunk and chunk-local coordinates, solid, light-blocking, and liquid flags, plus the resolved liquid connectivity-group label, liquid cardinal mask, liquid animation frame index, frame count, frame duration, elapsed-in-frame time, normalized frame-progress percentage, remaining-in-frame time, loop duration, elapsed-in-loop time, normalized loop-progress percentage, remaining-in-loop time, and current liquid variant source, UV rect, and atlas pixel bounds for liquid tiles.
...
- App shell: click `Enter World` from the main menu after boot finishes, then use `Resume World (Enter)` after returning to the menu from an active session or `New World (N)` from that paused menu to discard the current session and boot a fresh world immediately; the paused-menu shell copy warns that `New World` also clears the current session camera state and undo history before the fresh world boots, and the `New World` button tooltip repeats that reset warning together with the `N` shortcut; while that paused menu is visible, press `Enter` to trigger the same resume path from the keyboard or `N` to trigger the fresh-world reset path.
- In-world shell: click `Main Menu (Q)` or press `Q` to pause back to the main menu without discarding the current world session, click `Recenter Camera` or press `C` to snap the camera back to the standalone player and clear any preserved manual follow offset, click `Show Debug HUD` or `Hide Debug HUD` or press `H` to toggle the text telemetry overlay, click `Show Edit Panel (G)` or `Hide Edit Panel (G)` or press `G` to toggle the full `Debug Edit` control surface, click `Show Edit Overlays` or `Hide Edit Overlays` or press `V` to toggle the compact debug-edit overlays, and click `Show Spawn Marker` or `Hide Spawn Marker` or press `M` to toggle the resolved spawn overlay.
