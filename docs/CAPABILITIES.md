...
- Deployment regression coverage temp-builds the production app, verifies the `/deep-factory/` bundle URLs, asserts that the emitted JavaScript bundle retains exactly one occurrence of the exact joined runtime atlas fetch URL `/deep-factory/atlas/tile-atlas.png` while omitting the exact legacy root-relative `'/atlas/tile-atlas.png'` literal, and byte-compares the copied authored atlas output at `dist/atlas/tile-atlas.png` against `public/atlas/tile-atlas.png`.
