...
- The default tile set now also includes placeholder `water` and `lava` brush tiles with dedicated `liquidRender` metadata, separate liquid connectivity groups, and animated liquid frame sequences; water now resolves distinct placeholder surface, edge, and body sources from liquid masks through two-frame direct-`uvRect` animation in dedicated authored regions, while lava still reuses the same authored frame pair across masks until task 117 lands distinct lava edge and surface art.
