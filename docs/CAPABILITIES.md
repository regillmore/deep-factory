...
- Renderer draws the standalone player through a facing-aware world-space placeholder pass with grounded idle, alternating grounded-walk, jump-rise, fall, airborne wall-slide, and airborne ceiling-bonk silhouettes instead of a client-space DOM marker.
...
- Debug overlay shows FPS, rendered chunk count, mesh telemetry, loaded atlas source kind plus atlas dimensions, atlas validation status, the resolved standalone player spawn tile plus world coordinates, live standalone player state telemetry (`position`, `velocity`, `grounded`, `facing`), the latest standalone player grounded-transition event (`jump`, `fall`, or `landing` with resolved position and velocity), the latest standalone player facing-transition event (`left->right` or `right->left` with resolved position and velocity), the latest standalone player auto-respawn event when embedded-tile recovery reuses the resolved spawn (`embedded` with spawn tile, position, and velocity), the latest standalone player wall-contact and ceiling-contact transition events (`blocked` or `cleared` with tile, position, and velocity), live standalone player collision AABB telemetry (`min`, `max`, `size`), live standalone player camera-follow telemetry (`focus`, `offset`), live standalone player collision-contact telemetry (`support`, `wall`, `ceiling` with tile coordinates and IDs when present), live standalone player input-intent telemetry (`move`, `jumpHeld`, `jumpPressed`), live pointer inspect data (`client`, `canvas`, `world`, hovered tile name plus tile ID, `tile`, `chunk`, `chunk-local`, `solid`, `light`, `liquid`), and a separate pinned tile metadata line when inspect pin is active.
...
- App shell: click `Enter World` from the main menu after boot finishes, then use `Resume World (Enter)` after returning to the menu from an active session or `New World (N)` from that paused menu to discard the current session and boot a fresh world immediately; while that paused menu is visible, press `Enter` to trigger the same resume path from the keyboard or `N` to trigger the fresh-world reset path.
