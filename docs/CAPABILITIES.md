...
- Standalone player-state helpers track feet-centered `position`, `velocity`, explicit `size`, `grounded`, and `facing`, can seed that state from spawn output, derive the collision AABB plus one-step fixed-update integration from the same model, can advance through x-then-y collision sweeps that zero blocked velocity and recompute grounded support after movement, and can apply gravity plus fall-speed clamping before the shared collision step.
- The fixed update loop now keeps a spawned standalone player state alive even before controller input exists, so removing support tiles in debug edit mode causes the player to fall and re-collide against terrain.
