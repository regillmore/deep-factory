...
- Deployment regression coverage temp-builds the production app, verifies the `/deep-factory/` bundle URLs, scans every emitted JavaScript asset to assert that the exact joined runtime atlas fetch URL `/deep-factory/atlas/tile-atlas.png` appears exactly once across the full emitted JS set while the exact legacy root-relative `'/atlas/tile-atlas.png'` literal appears in none of them, and byte-compares the copied authored atlas output at `dist/atlas/tile-atlas.png` against `public/atlas/tile-atlas.png`.
