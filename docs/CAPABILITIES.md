...
- Deployment regression coverage temp-builds the production app, verifies the `/deep-factory/` bundle URLs, scans emitted `index.html` to reject both root-relative `"/assets/..."` and double-prefixed `"/deep-factory/deep-factory/assets/..."` script and stylesheet URLs, scans every emitted CSS asset to reject the exact root-relative `"/assets/"` bundle literal, the exact double-prefixed `"/deep-factory/deep-factory/assets/"` bundle literal, and the legacy root-relative `'/atlas/tile-atlas.png'` literal, scans every emitted JavaScript asset to reject those same literals plus the exact double-prefixed `"/deep-factory/deep-factory/atlas/tile-atlas.png"` authored-atlas runtime URL literal while the exact joined runtime atlas fetch URL `/deep-factory/atlas/tile-atlas.png` still appears exactly once across the full emitted JS set, and byte-compares the copied authored atlas output at `dist/atlas/tile-atlas.png` against `public/atlas/tile-atlas.png`.
