...
blend helper that task `18` can consume when the renderer starts drawing interpolated entity
snapshots. The renderer and debug telemetry still read the current player state directly until
task `18` adds interpolated entity rendering.
