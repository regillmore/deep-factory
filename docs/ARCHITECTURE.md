...
- `src/world/`: world data model, chunk math, collision queries, spawn and player-state helpers, procedural generation, sparse edited-tile overrides that survive chunk streaming prune, resident per-chunk light storage plus dirty-light local-column invalidation that keeps edge-edit sunlight dirtiness in the edited `chunkX` column until horizontal transport exists, resident sunlight recomputation from exposed chunk tops, mesh construction, plus authored atlas-region layout data.
...
4. Recompute dirty resident chunk light fields by propagating sunlight top-down from exposed resident chunk tops only within resident chunk columns that currently contain dirty light chunks, while limiting recomputation to the dirty local sunlight columns collected for each resident `chunkX` column, before chunk rendering work consumes those caches.
