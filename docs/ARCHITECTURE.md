...
4. Recompute dirty resident chunk light fields by propagating sunlight top-down from exposed resident chunk tops only within resident chunk columns that currently contain dirty light chunks, then transport sunlight across loaded neighboring `chunkX` boundary columns on dirty edge columns, then merge local emissive source falloff over that sunlight base, while limiting those passes to the dirty local sunlight columns collected for each resident `chunkX` column before chunk rendering work consumes those caches.
