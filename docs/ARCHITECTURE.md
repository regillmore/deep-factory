...
- `src/main.ts` owns the current standalone-player orchestration: it seeds the player from the resolved spawn once, pulls shared desktop or touch movement intent from `src/input/controller.ts`, advances that state in fixed updates via a narrow renderer world-query wrapper only while the shell is `in-world`, folds manual pan or zoom camera deltas into a persistent follow offset, loads and saves in-world shell-toggle visibility through local persistence with all-off fallback defaults, lets the shell either pause back to the main menu without discarding the current session, abandon that paused session through a fresh-world reset that restores first-start runtime defaults while preserving initialized renderer/input and persisted debug-edit prefs, or recenter the camera by zeroing that offset and snapping back to the player's focus point, and passes the latest state plus current sided wall contact, ceiling contact, and a short presentation-only ceiling-bonk hold latch into the renderer's temporary world-space placeholder draw pass until entity rendering exists.
